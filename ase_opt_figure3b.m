...
%  figure 3b - introduction of T2

toN = 3:2:9; % total number of echoes
r2p = 3.5e-3; % [ms-1]
t2 = 80; % [ms]
r2 = 1/t2; % [ms-1]
TE = [0:0.01:300]'; % [ms] TE and tau_max
...
xlim([0 max(TE)])
ylim([0 0.04])
hlegend = legend(h,leg_str,'Location','northeast');
box on
% Vertical line at echo time
if 0
    yax = ylim;
    vert = 0:yax(2)/1000:yax(2);
    hold on, plot(ones(size(vert)).*t2, vert, 'k--');
end
